   ...
   public class Spy
    {
       ...
       public string RevealPrivateMethods(string className)
        ...
        public string CollectGettersAndSetters(string className)
        {
            ...
            foreach(var items in methods.Where(x=>x.Name.StartsWith("get")))
            {
                builder.AppendLine($"{items.Name} will return {items.ReturnType}");
            }
            foreach(var items in methods.Where(x=>x.Name.StartsWith("set")))
            {

                var param = items.ReturnParameter;

                builder.AppendLine($"{items.Name} will set field of {items.GetParameters().First().ParameterType}");
            }
